...
MATERIALS = {
    ...
    # Tier 1 — first processing  (prices computed at load from INPUT_RATIO)
...
def _compute_prices():
    """Fill in prices for all non-raw materials: 2 × INPUT_RATIO × sum(input prices).
    Raw materials keep their hardcoded price. Runs bottom-up so dependencies resolve."""
    done = {m for m in MATERIALS if "price" in MATERIALS[m]}
    todo = [m for m in MATERIALS if m not in done]
    while todo:
        for mat in list(todo):
            inputs = BUILDINGS[PRODUCER[mat]]["inputs"]
            if all(i in done for i in inputs):
                MATERIALS[mat]["price"] = 2 * INPUT_RATIO * sum(
                    MATERIALS[i]["price"] for i in inputs)
                done.add(mat)
                todo.remove(mat)
